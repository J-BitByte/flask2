...
def add_by_uuid():
    ...
    try:
        ...
        return{"messgae": "Data not defined"}, 500
...
@app.route("/count")
def count():
    # Returns number of items in the data list 
    
    return {"Data Count ": len(data)},200 

@app.route("/person/<var_name>")
def find_by_uuid(var_name):
    #iterate through the 'data list to search for the mathcing ID
    for person in data:
        #check if the id field matches the var_name
        if person["id"] == str(var_name):
            #return the person as a JSON response when found
            return person

    #return a JSON with a message and 404 if not matching
...
@app.route("/exp")
def index_explicit():
    res = make_response("Explicit Response")
    res.status_code = 200
    return res
